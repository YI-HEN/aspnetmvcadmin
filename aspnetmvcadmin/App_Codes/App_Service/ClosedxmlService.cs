...
public class ClosedxmlService : BaseClass
{
    ...
    public void SaveFile(XLWorkbook excelWorkbook)
    {
        ...
        SaveFile(excelWorkbook, ExcelPath, str_file_name);
    }
    ...
    public void SetHeader(ref IXLWorksheet ws, int rowIndex, int colSpan, ExcelHeader header)
    {
        ws.Range(rowIndex, 1, rowIndex, colSpan).Merge();
    ...
    /// <param name="columns">合併欄位數</param>
    /// <param name="textValue">條件列文字</param>
    public void SetCondition(ref IXLWorksheet ws, int rowIndex, int columns, ExcelHeader header)
    {
        ws.Range(rowIndex, 1, rowIndex, columns).Merge();
        ws.Cell(rowIndex, 1).Value = header.Title;
        if (header.FontBold) ws.Cell(rowIndex, 1).Style.Font.SetBold();
    ...
    public void SetCaption(ref IXLWorksheet ws, int rowIndex, List<ExcelColumn> columns)
    {
        int index = 0;
    ...
    /// <param name="columns">欄位陣列</param>
    public void SetCellValue(ref IXLWorksheet ws, int rowIndex, dynamic item, List<ExcelColumn> columns)
    {
        int index = 0;
        foreach (ExcelColumn column in columns)
        {
